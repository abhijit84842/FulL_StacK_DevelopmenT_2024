...
const url =
  "mongodb+srv://abhijitbackend:abhijit84842@cluster0.l4vqt9f.mongodb.net/JwtAuthDB?retryWrites=true&w=majority&appName=Cluster0";
...
app.post("/login", async (req, res) => {
  ...
  bcrypt.compare(req.body.password, user.password, function (err, result) {
    // result == true
    if (result) {
      let token = jwt.sign({ email: user.email }, "secrectkey");        // create jwt token to create encrypted cookie
      res.cookie("token", token);
